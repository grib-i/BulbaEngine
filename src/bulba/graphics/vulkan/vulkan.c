...
static int pick_device(VULKAN*v){uint32_t n=0;vkEnumeratePhysicalDevices(v->instance,&n,NULL);if(!n)return -1;VkPhysicalDevice*ds=malloc(sizeof(*ds)*n);if(!ds)return -1;vkEnumeratePhysicalDevices(v->instance,&n,ds);for(uint32_t i=0;i<n;i++){uint32_t qn=0;vkGetPhysicalDeviceQueueFamilyProperties(ds[i],&qn,NULL);VkQueueFamilyProperties*q=malloc(sizeof(*q)*qn);vkGetPhysicalDeviceQueueFamilyProperties(ds[i],&qn,q);for(uint32_t j=0;j<qn;j++){VkBool32 present=VK_FALSE;vkGetPhysicalDeviceSurfaceSupportKHR(ds[i],j,v->surface,&present);if((q[j].queueFlags&VK_QUEUE_GRAPHICS_BIT)&&present){v->physical_device=ds[i];v->graphics_queue_family=j;free(q);free(ds);return 0;}}free(q);}free(ds);return -1;}
