...
static void destroy_font_resource(VULKAN *vulkan, VULKAN_FontResource *resource) {
  ...
  if (resource->descriptor_pool != VK_NULL_HANDLE) {
    vkDestroyDescriptorPool(vulkan->device, resource->descriptor_pool, NULL);
  ...
  if (resource->sampler != VK_NULL_HANDLE) {
    vkDestroySampler(vulkan->device, resource->sampler, NULL);

    resource->sampler = VK_NULL_HANDLE;
  }

  if (resource->view != VK_NULL_HANDLE) {
    vkDestroyImageView(vulkan->device, resource->view, NULL);
...
static VULKAN_FontResource *find_font_resource(VULKAN *vulkan, const Font *font) {
  ...
  for (size_t i = 0; i < vulkan->font_count; i++) {
    ...
    if (vulkan->fonts[i].font == font)
      return &vulkan->fonts[i];
  }
...
int VULKAN_RendererLoadFont(VULKAN *vulkan, const Font *font) {
  ...
  if (find_font_resource(vulkan, font))
    return 0;

  if (vulkan->font_count >= VULKAN_MAX_FONTS)
    return -1;

  if (vkDeviceWaitIdle(vulkan->device) != VK_SUCCESS)
    return -1;

  VULKAN_FontResource *resource = &vulkan->fonts[vulkan->font_count];

  memset(resource, 0, sizeof(*resource));

  if (create_font_texture(vulkan, font, resource) != 0) {
...
void VULKAN_RendererUnloadFont(VULKAN *vulkan) {
  ...
  memset(vulkan->fonts, 0, sizeof(vulkan->fonts));
...
void VULKAN_RendererDrawText(VULKAN *vulkan, const Font *font, const char *text, float x, float y, float glyph_scale, HMM_Vec2 transform_scale,
  ...
  VULKAN_FontResource *font_resource = find_font_resource(vulkan, font);
  ...
  if (vulkan->text_vertex_cursor >= VULKAN_MAX_TEXT_VERTICES)
    return;
  ...
  while (*cursor != '\0') {
    ...
    vertices[index + 0] = (VulkanTextVertex){
        .position =
            {
                px,
                py,
    ...
    vertices[index + 1] = (VulkanTextVertex){
        .position =
            {
                px + width,
                py,
    ...
    vertices[index + 2] = (VulkanTextVertex){
        .position =
            {
                px + width,
                py + height,
    ...
    vertices[index + 3] = (VulkanTextVertex){
        .position =
            {
                px,
                py,
    ...
    vertices[index + 4] = (VulkanTextVertex){
        .position =
            {
                px + width,
                py + height,
    ...
    vertices[index + 5] = (VulkanTextVertex){
        .position =
            {
                px,
                py + height,
  ...
  vkCmdBindDescriptorSets(command, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, &font_resource->descriptor_set, 0, NULL);
