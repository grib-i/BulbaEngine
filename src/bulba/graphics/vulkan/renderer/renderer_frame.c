...
int create_depth_resources(VULKAN *vulkan) {
  if (!vulkan || vulkan->swapchain_image_count == 0)
    return -1;
...
int create_framebuffers(VULKAN *vulkan) {
  if (!vulkan || vulkan->device == VK_NULL_HANDLE || vulkan->render_pass == VK_NULL_HANDLE || vulkan->swapchain_image_count == 0 ||
      !vulkan->swapchain_image_views || !vulkan->depth_image_views)
    return -1;
  ...
  for (uint32_t i = 0; i < vulkan->swapchain_image_count; i++) {
    if (vulkan->swapchain_image_views[i] == VK_NULL_HANDLE || vulkan->depth_image_views[i] == VK_NULL_HANDLE)
      goto fail;
...
int VULKAN_RendererBeginFrame(VULKAN *vulkan) {
  ...
  if (vulkan->current_image >= vulkan->swapchain_image_count)
    return -1;
...
void VULKAN_RendererBeginMainPass(VULKAN *vulkan) {
  ...
  VkFramebuffer framebuffer = vulkan->framebuffers[vulkan->current_image];
...
int VULKAN_RendererEndFrame(VULKAN *vulkan) {
  ...
  VkSubmitInfo submit = {
      ...
      .pWaitSemaphores = &vulkan->image_available[frame],
      .pWaitDstStageMask = &wait_stage,
      .commandBufferCount = 1,
      .pCommandBuffers = &command,
      .signalSemaphoreCount = 1,
      .pSignalSemaphores = &vulkan->render_finished[frame],
  };

  if (vkQueueSubmit(vulkan->graphics_queue, 1, &submit, vulkan->in_flight[frame]) != VK_SUCCESS)
    return -1;

  VkPresentInfoKHR present = {
      .sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,
      .waitSemaphoreCount = 1,
      .pWaitSemaphores = &vulkan->render_finished[frame],
      .swapchainCount = 1,
...
int VULKAN_RendererRecreateSwapchain(VULKAN *vulkan, bool vsync) {
  ...
  if (VULKAN_CreateSwapchain(vulkan, vsync) != 0)
    return -1;
