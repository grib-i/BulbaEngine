...
float spot_factor(vec3 direction_to_light, vec3 spot_direction, float inner_cone, float outer_cone) {
  ...
  float theta = dot(direction_to_light / direction_length, spot_direction / spot_length);
...
float shadow_visibility(vec3 world_position, vec3 normal, vec3 light_direction) {
  ...
  vec4 shadow_clip = shadow_mvp * vec4(world_position, 1.0);
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      visibility += texture(shadow_map, vec3(uv + vec2(x, y) * texel, depth));
    }
  }
...
void main() {
  ...
  float lighting_enabled = push_constants.material.x;
  ...
  for (uint i = 0u; i < light_count && i < MAX_LIGHTS; i++) {
    ...
    if (light.cone.w == LIGHT_DIRECTIONAL) {
      direction_to_light = normalize(-light.direction.xyz);
      casts_shadow = true;
    } else if (light.cone.w == LIGHT_POINT || light.cone.w == LIGHT_SPOT) {
      ...
      if (light.cone.w == LIGHT_SPOT)
        attenuation *= spot_factor(-direction_to_light, light.direction.xyz, light.cone.y, light.cone.z);
    }

    if (attenuation <= 0.0)
      continue;

    float ndotl = max(dot(normal, direction_to_light), 0.0);
    float diffuse = ndotl * ndotl * (3.0 - 2.0 * ndotl);
    float shadow = casts_shadow && diffuse > 0.0 ? shadow_visibility(in_position, normal, direction_to_light) : 1.0;

    ambient_light += ambient * intensity * attenuation;
    diffuse_light += light_color * intensity * diffuse * attenuation * shadow;
  }

  vec3 lit_color = base * (ambient_light + diffuse_light);
  vec3 self_lit = base * (emission + glow);
  vec3 final_color = lit_color + self_lit;

  out_color = vec4(min(final_color, vec3(1.0)), in_color.a);
}
